...
// NewConsumer creates a new Kafka Consumer for the notification-service.
func NewConsumer(brokers []string, groupID string, service *app.NotificationService, logger *zap.Logger) (*Consumer, error) {
...
func (c *Consumer) Start(ctx context.Context) {
	c.logger.Info("notification-service kafka consumer started")
	for {
...
func (c *Consumer) handleRecord(ctx context.Context, record *kgo.Record) {
	...
	default:
		c.logger.Warn("unexpected topic", zap.String("topic", record.Topic))
	}
}
